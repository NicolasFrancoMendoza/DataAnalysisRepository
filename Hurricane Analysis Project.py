...
def update_recorded_damages(lists):
  update_recorded = []
  for value in  lists:
    if "M" in value:
      vfloat = value.replace("M","") #nuevo string, solo numeros
      value = float(vfloat)
      update_recorded.append(value*conversion["M"])
    elif "B" in value:
      vfloat = value.replace("B","") #nuevo string, solo numeros
      value = float(vfloat)
      update_recorded.append(value*conversion["B"])
    else: 
      update_recorded.append(value)
      
  return update_recorded
